...
#include "klee/util/Mutex.h" // commit 6d8861f5
...
#include <assert.h>
#include <string.h>
#include <time.h> // commit 329130e9

#include <set>
#include <vector> // commit 48dfa31
#include <ostream> // commit 537dad89
...
namespace klee { // commit fe5b5b217
  Mutex logging_mutex;
}
...
static void klee_vfmessage(FILE *fp, const char *pfx, const char *msg,
  ...
  fdos << timebuff << " | KLEE: "; // AC: added timestamp
  if (pfx)
    fdos << pfx << ": ";
...
// commits 48dfa3102, 329130e9, 537dad8994, 4f5abafa, 5972e94e, 97f588d63
...
static void klee_vmessage(const char *pfx, bool onlyToFile, const char *msg,
                          va_list ap) {
  LockGuard guard(klee::logging_mutex); // commit fe5b5b21
  if (klee_warning_stream && klee_message_stream) { // commit 51bbe78, 537dad89
...
void klee::klee_warning_once(const void *id, const char *msg, ...) {
  static Mutex lock; // commit 6d8861f5a (FIXME: use logging_mutex?)
  static std::set<std::pair<const void *, const char *> > keys;
  std::pair<const void *, const char *> key;

  LockGuard guard(lock); // commit 6d8861f5a (FIXME: use logging_mutex?)
