...
// RUN: %klee -cliver -search-mode=pq -use-full-variable-names=1 -optimize=0 -posix-runtime -libc=uclibc -no-output -socket-log=%t1.ktest %t1.bc &> %t1.cliver.log
// RUN: grep -q "CLIENT: success" %t1.cliver.log
// UN: mkdir -p %t2.cliver.dir
// UN: %klee -cliver -cliver-mode=training -optimize=0 -posix-runtime -libc=uclibc -output-dir-parent=%t2.cliver.dir -socket-log=%t1.ktest %t1.bc &> %t2.cliver.log
// UN: grep -q "CLIENT: success" %t2.cliver.log
// UN: mkdir -p %t3.cliver.dir
// UN: %klee -cliver -use-clustering -cliver-mode=edit-dist-kprefix-row -optimize=0 -posix-runtime -libc=uclibc -output-dir-parent=%t3.cliver.dir -training-path-dir=%t2.cliver.dir/cliver-last -socket-log=%t1.ktest %t1.bc &> %t3.cliver.log
