...
// Fourth set: prohibitive functions and lazy constraint generators for both the
// forward p() function and p_inv(). (should follow expected verifier result)
...
// RUN: rm -rf %t2.dir
// RUN: %klee --output-dir=%t2.dir -cliver -enable-lazy-constraints -optimize=0 -posix-runtime -libc=uclibc -socket-log=%t1.ktest %t1.bc -i -P -L pboth &> %t2.log
...
// RUN: rm -rf %t4.dir
// RUN: %klee --output-dir=%t4.dir -cliver -enable-lazy-constraints -optimize=0 -posix-runtime -libc=uclibc -socket-log=%t1.ktest %t1.bc -i -P -L pboth &> %t4.log
...
// RUN: rm -rf %t6.dir
// RUN: not %klee --output-dir=%t6.dir -cliver -enable-lazy-constraints -optimize=0 -posix-runtime -libc=uclibc -socket-log=%t1.ktest %t1.bc -i -P -L pboth &> %t6.log
