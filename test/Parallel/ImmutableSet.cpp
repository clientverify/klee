// RUN: %llvmgxx -I../../../include -g -DMAX_ELEMENTS=5 -fno-vectorize -fno-exceptions -emit-llvm -c -o %t1.bc %s
// RUN: rm -rf %t.klee-out
// RUN: %klee -use-threads=8 --use-batching-search=1 --use-forked-solver=0 -output-istats=0 --use-call-paths=0 --output-dir=%t.klee-out --libc=klee --max-forks=30 --no-output --exit-on-error --optimize --disable-inlining --search=nurs:depth --use-cex-cache %t1.bc
