...
int old_main(int argc, char **argv) {
  ...
  if (xc.init()) {
    llvm::cout << "XPilotChecker initialized.\n";
  } else {
    llvm::cerr << "XPilotChecker not initialized.\n";
  }
  xc.checkValidity();
  return 0;
}
...
namespace {
  ...
  OnlyErrorOutput("only-error-output", 
           cl::desc("Only generate test files for errors."));
  ...
  /* NUKLEAR KLEE begin */
  cl::list<std::string>
  SocketReplayFile("socket-replay",
                cl::desc("Specify a socket out file to replay"),
                cl::value_desc("out file"));

  cl::list<std::string>
  SocketReplayDir("socket-replay-dir",
	       cl::desc("Specify a directory to replay .out files from"),
	       cl::value_desc("output directory"));
  /* NUKLEAR KLEE end */
...
void KleeHandler::processTestCase(const ExecutionState &state,
  ...
  if (errorMessage && std::string(errorMessage) == std::string("writePC")) {
    if (errorSuffix) {
      std::ofstream *f = static_cast<std::ofstream*>(openOutputFile(errorSuffix));
  ...
  if (errorMessage && std::string(errorMessage) == std::string("writeCVC")) {
    if (errorSuffix) {
      std::ofstream *f = static_cast<std::ofstream*>(openOutputFile(errorSuffix));
      std::string constraints;
  ...
  if (OnlyErrorOutput) {
    if (errorMessage) {
      ...
      std::ofstream *f = static_cast<std::ofstream*>(openTestFile(errorSuffix, id));
      if (f) {
        *f << errorMessage;
        f->flush();
        f->close();
...
int main(int argc, char **argv, char **envp) {  
  ...
  if (!ReplayOutDir.empty() || !ReplayOutFile.empty()) {
    // removed handling of replays
  } else {

    // removed handling of seeds
    ...
    std::vector<std::string> replayFiles = SocketReplayFile;
    for (std::vector<std::string>::iterator
           it = SocketReplayDir.begin(), ie = SocketReplayDir.end();
         it != ie; ++it)
      KleeHandler::getOutFiles(*it, replayFiles);    

    std::vector<KTest*> kTests;
    for (std::vector<std::string>::iterator
           it = replayFiles.begin(), ie = replayFiles.end();
         it != ie; ++it) {
      KTest *out = kTest_fromFile(it->c_str());
      if (out) {
        kTests.push_back(out);
        std::cerr << "KLEE: opened KTest file: " << *it 
          << ", with " << out->numObjects << " objects.\n";
      } else {
    ...
    for (std::vector<KTest*>::iterator
           it = kTests.begin(), ie = kTests.end(); it != ie; ++it) {

      KTest *out = *it;
      interpreter->addSocketKTest(i--, out);
    }
